...
def web_page(input_values=[None,None]):
    ...
    html = html.replace('xx_data_xx',str(input_values[2]))
    return html
...
def serve(access_point):
    ...
    while True:
      try:
        ...
        end_input_index = request.find('ENDI')
        date_time = request[data_index+12:end_time_index]
        date_time = date_time.replace('%20', ' ')
        data_label = request[end_time_index+4:end_input_index]
        ...
        response = web_page([date_time, data_label, total_buffer])
